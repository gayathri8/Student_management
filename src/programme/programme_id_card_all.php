...
$TBS->LoadTemplate('programme_id_card_all.html'); 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	function generatePdf($conn, $enroll_list, $program_id, $batch_year, $count, $pdf, $fonts) {
		$y_axis = 0;
		$x_axis = 0;
	...
	$stmt = $conn->prepare("SELECT * FROM student WHERE program_id = :program_id AND year = :year");
	$stmt->execute(array(':program_id' => $program_id, ':year' => $year));
	$row = $stmt->fetchAll();
	...
	if (count($row)) {
		...
		foreach ($row as $key => $value) {
			...
			generatePdf($conn, $enroll, $program_id, $year, $count, $pdf, $fonts);
			$count++;
		}

		$file = $program_id."_IdCard.pdf";
		$pdf->Output('I', $file);
	} else {
		$msg_err = "Year of joining is invalid.";
	}
}
