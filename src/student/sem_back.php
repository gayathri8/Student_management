...
$TBS->LoadTemplate('sem_back.html'); 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['student_roll_no']) && (isAlphaNum($_POST['student_roll_no']))) {
	...
	if (isset($_POST['sem_in']) && filter_var($_POST['sem_in'],FILTER_VALIDATE_INT)) {
		$sem_in = $_POST['sem_in'];
	}
	...
	if (isset($_POST['sem_go_to']) && filter_var($_POST['sem_go_to'],FILTER_VALIDATE_INT)) {
		$sem_go_to = $_POST['sem_go_to'];
	}
	...
	$program_id = get_val('student', 'enrollment_no', $enrollment_no, 'program_id');
	if (!$program_id) {
		$msg_err = "The student doesn't belong to the entered program.";
	}
	...
	if (!$msg_err) {
		try {
			...
			$sql = "INSERT INTO `sem_back`(`program_id`, `student_id`, `year_in`, `sem_in`, `year_go_to`, `sem_go_to`, `log_id`, `status_id`) VALUES ('$program_id','$student_id','$year_in','$sem_in','$year_go_to','$sem_go_to','LOG_ID','$status_id')";

			$ac_on = "Entered a new semester back for ".$enrollment_no."in ".$sem_in;
