...
$TBS->LoadTemplate('student_allot_hostel.html');
	...
	if ($_FILES["file"]["error"] > 0) {
		...
		if (!feof($uploadedFP)) {
			while ($data = fgetcsv($uploadedFP)) {
				...
				$enroll_no = filter_var($data[0], FILTER_SANITIZE_STRING);
				$hostel = filter_var($data[1], FILTER_SANITIZE_STRING);
				$hostel_room = filter_var($data[2], FILTER_SANITIZE_STRING);

				$student_id = get_val('student', 'enrollment_no', $enroll_no, 'student_id');
				if (!$student_id) {
		...
		if (!feof($uploadedFP) && $all_correct) {
			while ($data = fgetcsv($uploadedFP)) {
				...
				$ac_on = "Updated hostel for $enrollment_no to $hostel and room number to $hostel_room";
				...
				$res['hostel_no'] = $hostel;
				$res['hostel_room'] = $hostel_room;
				$res['log_id'] = $log_id;
