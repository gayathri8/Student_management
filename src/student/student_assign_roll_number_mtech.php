...
$TBS->LoadTemplate('student_assign_roll_number_mtech.html');
	...
	if ($_FILES["file"]["error"] > 0) {
		...
		if (!feof($uploadedFP) && $all_correct) {
			while ($data = fgetcsv($uploadedFP)) {
				...
				$sql = "UPDATE student SET enrollment_no=:enrollment_no WHERE student_id=:student_id";

				$stmt = $conn->prepare($sql);

				$stmt->bindParam(':enrollment_no', $student_roll_no);
				...
				// $stmt = $conn->prepare($sql);
				// $stmt->bindParam(':enrollment_no', $registration_no);
				// $stmt->execute();
				...
				//   $ac_on = "Updated roll number for $registration_no to $student_roll_no.";
				...
				//   $res['student_id'] = NULL;
				//   $res['enrollment_no'] = $student_roll_no;
				//   $res['log_id'] = $log_id;

				//   var_dump($res['program_id']);
